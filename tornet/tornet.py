...
def rotate_tor_ip():
    """Rotate the Tor exit IP by sending NEWNYM to the local control port.
    Works without root: user 'd' is in the debian-tor group and the auth
    cookie at /run/tor/control.authcookie is group-readable, so we can
    authenticate to 127.0.0.1:9051 directly instead of shelling out to sudo."""
    try:
        ...
        warning(f"Could not rotate IP via control port: {e}")
        return False
